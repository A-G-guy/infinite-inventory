...
- 支持类 JEI 的特殊语法：`@模组`、`#物品标签`、`&物品 ID`、`%创造标签页`，并可配合 `|` 做或查询、`-` 做排除。
...
- 特殊搜索支持 `@` 模组筛选、`#` 物品标签筛选、`&` 注册名筛选、`%` 创造标签页筛选，空格默认为 AND，`|` 为 OR，`-` 为排除。
- 多词模组名和创造标签页在不歧义时可直接输入，例如 `@infinite inventory`、`%building blocks`。
