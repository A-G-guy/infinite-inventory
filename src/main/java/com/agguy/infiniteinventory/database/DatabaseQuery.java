...
public record DatabaseQuery(
    ...
    public static DatabaseQuery defaultQuery(DatabaseScope scope) {
        return new DatabaseQuery(scope, DatabaseCategory.ALL, DatabaseSortOption.RECENTLY_CHANGED, "", 0, DEFAULT_PAGE_SIZE);
    }
    ...
    public DatabaseQuery withCategory(DatabaseCategory newCategory) {
        return new DatabaseQuery(this.scope, newCategory, this.sortOption, this.searchText, 0, this.pageSize);
    }

    public DatabaseQuery withSortOption(DatabaseSortOption newSortOption) {
        return new DatabaseQuery(this.scope, this.category, newSortOption, this.searchText, this.pageIndex, this.pageSize);
    }

    public DatabaseQuery withSearchText(String newSearchText) {
        return new DatabaseQuery(this.scope, this.category, this.sortOption, newSearchText, 0, this.pageSize);
    }

    public DatabaseQuery withPageIndex(int newPageIndex) {
        return new DatabaseQuery(this.scope, this.category, this.sortOption, this.searchText, newPageIndex, this.pageSize);
    }

    public DatabaseQuery withPageSize(int newPageSize) {
        return new DatabaseQuery(this.scope, this.category, this.sortOption, this.searchText, this.pageIndex, newPageSize);
    }

    public DatabaseQuery withScope(DatabaseScope newScope) {
        return new DatabaseQuery(newScope, this.category, this.sortOption, this.searchText, this.pageIndex, this.pageSize);
    }
