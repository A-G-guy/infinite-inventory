...
public final class AccessoriesCompat {
    ...
    private static final AccessoriesCompatBridge BRIDGE = createBridge();
    ...
    private static AccessoriesCompatBridge createBridge() {
        if (!ModList.get().isLoaded(MOD_ID)) {
            return new NoOpAccessoriesCompatBridge();
        }
        try {
            Class<?> bridgeClass = Class.forName("com.agguy.infiniteinventory.compat.AccessoriesCompatBridgeImpl");
            return (AccessoriesCompatBridge) bridgeClass.getDeclaredConstructor().newInstance();
            ...
            LOGGER.error("无法初始化 Accessories 兼容桥接，已回退为禁用状态。mod={}", InfiniteInventory.MODID, exception);
            return new NoOpAccessoriesCompatBridge();
        } catch (LinkageError error) {
            LOGGER.error("Accessories 兼容桥接加载失败，可能是 API 版本不兼容，已回退为禁用状态。mod={}", InfiniteInventory.MODID, error);
            return new NoOpAccessoriesCompatBridge();
        }
    }
}
